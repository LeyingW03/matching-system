"""
plot_revenue_vs_price.py

基于现有仓库中 revenue.py / goldenmethod.py 的仿真框架，
对一系列价格 p 分别求解不动点 pi_m*，并统计平台的平均收益 (avg_revenue)，
最终画出 revenue（收益）随价格 P 变化的曲线图。

依赖：revenue.py 中的 simulate() 和 fixed_point_solve()
      （这里直接复用，不需要额外修改原始仿真逻辑）
"""
...
# ---------------------------------------------------------------------
# 1. 仿真过程（与 revenue.py 中的 simulate 保持一致）
# ---------------------------------------------------------------------
